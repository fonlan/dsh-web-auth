 ...
 */
function createHarness(withPassword, beforeGate, listen) {
    let now = Date.now();
    let port = 0;
    let secret = randomBytes(32);
    let hash = withPassword ? hashPassword(PASSWORD) : undefined;
    const state = {
        get secret() {
            return secret;
        },
    ...
    const isTrustedApiRequest = (req, trusted) => {
        const host = String(req.headers.host ?? '');
        const hostname = host.split(':')[0];
        const loopback = hostname === '127.0.0.1' || hostname === '::1' || hostname === 'localhost';
        if (!loopback && !trusted.includes(host) && !trusted.includes(hostname))
            return false;
...
async function rawUpgrade(port, cookie, host, path = '/api/mux/events') {
    ...
    for (let attempt = 0;; attempt++) {
        try {
            ...
            if (attempt === 1 || error.code !== 'EADDRNOTAVAIL')
                throw error;
...
describe('login flow', () => {
    ...
    it('honors X-Forwarded-For from a loopback peer for rate-limit identity', async () => {
        const h = createHarness(true);
        const fail = (xff) => h.request('/login', {
            method: 'POST',
